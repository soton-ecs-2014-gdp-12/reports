...
removed? ||| I believe its trying to convey the process we worked on creating the solution, however Im not sure the reasoning ~Chewett}
...
\todo{This needs the image fixing so the text isnt crushed together}
...
\begin{tikzpicture}[
  ...
  \draw[between] (videogular) --
      node[midway,above] {commands}
      node[midway,below] {video state} (vg-questions);
  \draw[between] (vg-questions) --
      node[midway,above] {commands}
      node[midway,below] {user interactions} (questions-worker);
  \draw[between] (questions-worker) --
      node[midway,above] {quiz description}
      node[midway,below] {utility function} (quiz);
\end{tikzpicture}
...
\todo{Autoref seems to fail to nicely add text for the lstlisting labels}
