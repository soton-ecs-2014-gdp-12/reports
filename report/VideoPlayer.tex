...
Several improvements to Videogular were made during the course of the project primarily focussing on accessibility. With these changes the Videogular player is now accessible to users who only navigate websites using the keyboard. All changes noted have been adopted by the Videogular project. Compatibility with a number platforms and devices were reviewed and the only main problem found was issues with the standard web browser in iOS which cannot be fixed due to device limitations.

We created two \gls{Videogular} plugins to allow us to add statistical data to the scrub bar and overlay marks (cuepoints) onto this. Both of these have been designed for use in our example websites, cuepoints for points where questions are and the heatmap to demonstrate how often sections of a video have been watched.
