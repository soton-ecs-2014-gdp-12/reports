...
Abstracting all calls using \gls{REST} means that other services will be able to interact with the back-end services in a language independent way, which will also make the application standalone (\cref{Req:Standalone}). To facilitate this, all \gls{REST} responses are returned with a \texttt{Cross-Origin} header. This allows servers which are not on the same machine to communicate with the \gls{REST} service.

These features will allow an external application to run the Web server separately and communicate with our server side code. The only dependency added by those who use our code will be that they need to be able to make \gls{REST} calls.
